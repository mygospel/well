<!DOCTYPE html>
<html lang="ko">
<meta content="text/html;charset=utf-8" http-equiv="Content-Type">
<meta content="IE=edge" http-equiv="X-UA-Compatible">
<meta content="width=device-width,initial-scale=1.0,minimum-scale=1.0,maximum-scale=1.0,user-scalable=no"
      name="viewport">
<head>
    <meta charset="UTF-8">
    <title>인터서브</title>
    <link href="../css/reset.css" rel="stylesheet" type="text/css"/>
    <link href="../css/ui.css" rel="stylesheet" type="text/css"/>
    <link href="../css/style.css" rel="stylesheet" type="text/css"/>
    <link href="../css/jquery-ui.css" rel="stylesheet" type="text/css"/>
    <link href="../css/swiper.min.css" rel="stylesheet" type="text/css"/>
</head>
<body>
<div class="container page-with-btn" style=";">

    <header>
        <div class="wrap">
            <article class="header-page-group">
                <a class="back-btn" href="#none"></a>
                <div class="page-txt">예약 상세</div>
            </article>
        </div>
    </header>

    <section class="my-shop-history-detail">

        <div class="inner wrap">

            <article class="history-detail-head" style="padding-bottom: 15px;">
                <div class="detail-head-box">
                    <div class="head-txt">인터서브 스터디 카페 대흥점</div>
                    <div class="state-box">
                        <div class="state-txt type-1">사용중</div>
                    </div>
                </div>
                <div class="detail-sub-box">
                    <div class="">1회 이용권 사용중</div>
                </div>
            </article>


            <article class="voucher-locker-group">

                <div class="seat-info-box">

                    <div class="info-item-box">
                        <i class="ic ic-info1"></i>
                        <div class="info-txt">예약됨</div>
                    </div>

                    <div class="info-item-box">
                        <i class="ic ic-info2"></i>
                        <div class="info-txt">선택 가능</div>
                    </div>

                    <div class="info-item-box">
                        <i class="ic ic-info3"></i>
                        <div class="info-txt">선택 됨</div>
                    </div>

                </div>

                <div class="locker-wrap">
                    <ul class="locker-list">

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-1</a>
                            </div>
                        </li>
                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn on" href="#none">A-2</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn type-off" href="#none">A-3</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn type-off" href="#none">A-4</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-5</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-6</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-7</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-8</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-9</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-10</a>
                            </div>
                        </li>


                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-6</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-7</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-8</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-9</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-10</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-6</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-7</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-8</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-9</a>
                            </div>
                        </li>

                        <li class="locker-box">
                            <div class="item-wrap">
                                <a class="item-btn" href="#none">A-10</a>
                            </div>
                        </li>

                    </ul>
                </div>

                <div class="locker-option">

                    <a class="prev-btn" href="#none"></a>
                    <div class="locker-txt">1층 복도 A구역</div>
                    <a class="next-btn" href="#none"></a>
                </div>

                <div class="locker-time-box">

                    <div class="time-box">
                        <div class="time-tit-box">예약 시작 / 종료 시간</div>
                        <a class="time-btn" href="javascript:selectOn('time1');" data-select="select1">9:00 / 18:00</a>
                    </div>

                </div>

                <div class="locker-menu-box">
                    <a class="item-btn type-1" href="#none">사물함<br/>비우기</a>
                    <a class="item-btn type-3" href="javascript:popupOpen('alert')">연장하기</a>
                </div>

            </article>


        </div>


    </section>

    <article class="page-with-btn">
        <!--		<article class="menu-btn-box">-->
        <!--			<a href="#none" class="menu-btn">확인</a>-->
        <!--		</article>-->
        <article class="menu-btn-box">
            <div class="menu-btn off">예약 내용 변경</div>
        </article>
    </article>


    <!-- 팝업 -->
    <article class="popup popup-msg" style="">

        <div class="popup-wrap">

            <div class="popup-msg-group">

                <div class="msg-contents-box">

                    <div class="ic-box">
                        <i class="ic ic-warning"></i>
                    </div>
                    <div class="contents-box">
                        <div class="txt-box">
                            정말 퇴실하시겠어요?<br/>
                            퇴실을 하게 되면 다시 입실이 불가능해요.
                        </div>
                    </div>

                </div>

                <div class="msg-btn-box">
                    <a class="msg-btn type-off" href="#none" onclick="popupDirectClose(this);">취소</a>
                    <a class="msg-btn" href="#none" onclick="popupDirectClose(this);">확인</a>
                </div>

            </div>
        </div>

    </article>

    <article class="popup popup-alert" style="">

        <div class="popup-wrap">

            <div class="popup-msg-group">

                <div class="msg-contents-box">

                    <div class="ic-box">
                        <i class="ic ic-alert"></i>
                    </div>
                    <div class="contents-box">
                        <div class="txt-box type-center">
                            <div class="accent-box">
                                1회 이용권 구매 시 구매 가능한<br/>사물함 기간을 이미 구매하셨습니다.
                            </div>
                        </div>
                    </div>

                </div>

                <div class="msg-btn-box">
                    <a class="msg-btn" href="#none" onclick="popupDirectClose(this);">확인</a>
                </div>

            </div>
        </div>

    </article>


    <!--
    selectOn 에서 data-foot의 value값을 지정하여 푸터 팝업 표시
    selectClick 에서 두번째 파라미터는 텍스트값을 변경할 data-select의 항목을 찾아서 변경함
    -->
    <article class="time-foot-item" data-foot="time1">

        <div class="wrap">

            <a class="bar" href="#none"></a>

            <div class="item-wrap">

                <div class="tit-box">예약 시작 시간</div>
                <div class="item-list">
                    <div class="item-box"><a class="item-btn" href="#none" onclick="selectClick(this,'select1')">9:00</a></div>
                    <div class="item-box"><a class="item-btn type-off" href="#none">10:00</a>
                    </div>
                    <div class="item-box"><a class="item-btn type-off" href="#none">11:00</a>
                    </div>
                    <div class="item-box"><a class="item-btn type-off" href="#none">12:00</a>
                    </div>
                    <div class="item-box"><a class="item-btn" href="#none" onclick="selectClick(this,'select1')">13:00</a></div>
                    <div class="item-box"><a class="item-btn" href="#none" onclick="selectClick(this,'select1')">14:00</a></div>
                    <div class="item-box"><a class="item-btn" href="#none" onclick="selectClick(this,'select1')">15:00</a></div>
                    <div class="item-box"><a class="item-btn" href="#none" onclick="selectClick(this,'select1')">16:00</a></div>
                    <div class="item-box"><a class="item-btn" href="#none" onclick="selectClick(this,'select1')">17:00</a></div>
                    <div class="item-box"><a class="item-btn" href="#none" onclick="selectClick(this,'select1')">18:00</a></div>
                </div>

            </div>
        </div>
    </article>


</div>

<script src="../js/jquery-3.1.1.min.js"></script>
<script src="../js/jquery-ui-1.12.1.js"></script>
<script src="../js/swiper.min.js"></script>
<script src="../js/ui.js"></script>
<script src="../js/jquery.ui.touch-punch.min.js"></script>

<script>
    $('li.locker-box .item-btn').on('click',function (){
		if($(this).hasClass('type-off')) return false;

        if($(this).hasClass('on')){
            $(this).removeClass('on')
        }else{
            $('li.locker-box .item-btn').not('type-off').removeClass('on');
            $(this).addClass('on');
        }
	})



</script>

</body>
</html>